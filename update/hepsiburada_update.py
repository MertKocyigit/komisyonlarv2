...
# -*- coding: utf-8 -*-
"""
Hepsiburada Updater (PDF → Trendyol Format CSV)
------------------------------------------------
Kullanım örnekleri:

# PDF'ten başlat (önerilen - yeni format)
python -m update.hepsiburada_update \
  --pdf "C:\\Users\\CASPER\\Downloads\\1713435863999_ek.4-komisyon-listesi-v8-sastem-1.pdf" \
  --backup
...
def main():
    p = argparse.ArgumentParser(description="Hepsiburada updater (PDF→Trendyol Format CSV)")
    ...
    if pdf_path or use_hardcoded:
        logger.info("Yeni Hepsiburada extractor kullanılıyor...")

        cmd = ["python", "scripts/hepsiburada_extract_commissions.py",
               "--out-csv", str(out_csv)]
